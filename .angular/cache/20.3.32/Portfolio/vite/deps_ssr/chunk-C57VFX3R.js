...
function _document(injector) {
  ...
  if (config2 && config2.document) {
    document2 = typeof config2.document === "string" ? _enableDomEmulation ? parseDocument(config2.document, config2.url !== void 0 ? (
      ...
      parseUrl(config2.url, "http://localhost").href
    ) : void 0) : window.document : config2.document;
  } else {
...
function createServerPlatform(options) {
  ...
  startMeasuring(measuringLabel);
  const { document: document2, url } = options;
...
async function renderModule(moduleType, options) {
  const { document: document2, url, extraProviders: platformProviders, allowedHosts } = options;
...
//# sourceMappingURL=chunk-C57VFX3R.js.map
